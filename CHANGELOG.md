...
- The suggestions inherit every exemption of the public members check, and skip four more cases of their own: operators (which have no private spelling) and enum constants (whose identifier is observable at run time through `name` and `toString`); fields bound by a named `this.x` or `super.x` formal, which cannot be renamed at all since Dart forbids a named parameter starting with an underscore; instance members redeclared anywhere in the hierarchy of a type declared in another library, where a private name would compile but silently stop dispatch from reaching the override or the interface implementation, while a member no foreign subtype mentions is still suggested because a private member is inherited across libraries unchanged; and top level declarations of a library any consumer of the package can import, which covers both a file re-exported by a barrel and one that simply sits under `lib/` outside `lib/src`, since Dart lets a consumer import that directly whether or not anything exports it (this last cut drops the suggestion alone: whether the analyzed code references such a declaration is still reported by the unused check, unchanged).
